...
@Component({
  ...
  styleUrls: ['./manage-new.component.scss']
})
export class ManageNewComponent implements OnInit, OnDestroy {
  ...
  public manualId: any = null;
  public trainingManual: any = null;
  public manualSearch: any = "";
  public zoomEmail: any = "";
  ...
  public countryId;
  public domainId;
  public showManualsPopup = false;
  public userId;
  public user: any;
  public threadId: number = 0;
  public threadInfo: any = [];
  public contentType: any = 44;
  ...
  settingPrice: any = false;
  settingPercent: any = false;

  public navUrl: string = "market-place/training";
  public viewUrl: string = "market-place/view/";
  ...
  public miniumExpiryDateBird: any = new Date();
  ...
  birdPercentage: any;
  birdValue: any;
  manualDiscountPrice: any;
  isManualIncludedWithTraining: any;
  manualName: any = "";
  showBirdPriceValidation: any = false;
  showManualDiscountPriceValidation: any = false;
  ...
  manualSearchTimeout: any;
  manualSearchSub: Subscription;
  ...
  salesPersonsList;
  salesPersonName: any;
  salesPersonSelectedIds: any = [];
  workStreamIds = [];
  salesPersonNames: any = '';
  sameName: any = {};
  trainingsTitleListGet: any;
  // Resize Widow
  ...
  searchManual() {
    ...
    if(this.manualSearchSub) this.manualSearchSub.unsubscribe();
    this.manualSearchTimeout = setTimeout(() => {
      this.manualSearchSub = this.threadApi.apiGetAllManuals(this.domainId, false, this.manualSearch).subscribe((response) => {
        this.filteredManuals = response.data.marketPlaceData;
      }, error => {
  ...
  ngOnInit(): void {
    ...
    let navUrl = localStorage.getItem('marketNav');
  ...
  showManualsPopupFn(event) {
    this.showManualsPopup = event;
    this.manualSearch = "";
    this.filteredManuals = this.manualsData;
  }

  setManual(manual, discount) {
    this.trainingManual = manual;
    this.manualId = manual.id;
    this.manualName = manual.manualName;
    this.showManualsPopup = false;
    this.updateManualDiscountPriceValue(discount);
  }

  removeLinkedManualFn(event) {
    this.trainingManual = null;
    this.manualId = null;
    this.manualName = "";
    this.manualDiscountPrice = "";
    this.showManualDiscountPriceValidation = false;
  }

  updateManualDiscountPriceValue(event) {
    if(!parseInt(event)) {
      this.manualDiscountPrice = "";
      this.showManualDiscountPriceValidation = false;
    }
    if(this.trainingManual) {
      if (parseInt(this.trainingManual.discountPrice && this.trainingManual.discountPrice != '0' ? this.trainingManual.discountPrice : this.trainingManual.price) <= parseInt(event)) {
        this.manualDiscountPrice = "";
        this.showManualDiscountPriceValidation = true;
      } else {
        this.manualDiscountPrice = event;
        this.showManualDiscountPriceValidation = false;
      }
    } else {
      this.manualDiscountPrice = "";
  ...
  getThreadFields() {
    ...
    this.threadApi.apiGetMarketPlaceFields(this.threadApiData).subscribe((response) => {
      ...
      if (this.threadInfo) {
        this.signedupUsers = this.threadInfo['signedupUsers'] ? parseInt(this.threadInfo['signedupUsers']) : 0;
      }
      ...
      for(let sec of showingArray) {
        ...
        for(let f of fieldLists) {
          ...
          if(f.cellArray.length > 0) {
            ...
            if (f.apiFieldKey == 'trainingMode') {
              this.currentTrainingMode = f.selectedValues;
            }
            ...
            if ((this.showOnlineField || this.showInPersonField) || (f.apiFieldKey == 'startDate' || f.apiFieldKey == 'endDate' || f.apiFieldKey == 'startTime' || f.apiFieldKey == 'endTime' || f.apiFieldKey == 'expiryDateBirdPrice')) {
              if (f.selectedValues) {
                if (this.duplicateMarketPlace && (f.apiFieldKey == 'startDate' || f.apiFieldKey == 'endDate' || f.apiFieldKey == 'expiryDateBirdPrice')) {
                  ...
                  if (this.threadInfo['isBirdPrice'] && f.apiFieldKey == 'expiryDateBirdPrice') {
                    val = new Date();
                    f.dateVal = new Date();
                  }
                } else {
            ...
            if (f.apiFieldKey == "startDate" && this.threadId > 0) {
              this.startDate = moment(val).utc().format();
              this.startDate = new Date(val);
              this.miniumEndDate = moment(val).utc().format();
              ...
              this.miniumExpiryDateBird = moment(val).utc().format();
              this.miniumExpiryDateBird = new Date(val);
            }
      ...
      setTimeout(() => {
        ...
        let traingingObject = {
          val: this.currentTrainingMode,
          isChange: false
        }
        this.updateTrainingModeSteps(traingingObject);
        this.showBirdValues = this.threadInfo['isBirdPrice'];
        this.birdValue = this.threadInfo['birdPrice'];
        this.birdPercentage = this.threadInfo['birdPercentage'];
        this.updateIsBirdPriceValue(this.showBirdValues);
        this.threadApi.apiGetAllManuals(this.domainId, false, "").subscribe((response) => {
          this.manualsData = response.data.marketPlaceData;
        this.filteredManuals = response.data.marketPlaceData;
        // console.log(this.threadInfo['manualID'], this.firstTime);
          if(this.threadInfo['manualID'] && !this.firstTime) {
            let manual = this.manualsData.filter(manual => manual.id == this.threadInfo['manualID']);
            if(manual && manual.length) {
              manual = manual[0];
              let discountPriceManual = (parseInt((manual.discountPrice && manual.discountPrice != '0' ? manual.discountPrice : ((manual.price && manual.price != '0') ? manual.price : 0))));
              this.setManual(manual, (discountPriceManual > parseInt(this.threadInfo['manualDiscountPrice']) ? this.threadInfo['manualDiscountPrice'] : ''));
            }
          }
          if(this.threadInfo['salesPerson'] && !this.firstTime) {
            this.salesPersonSelectedIds = [];
            this.salesPersonData = this.threadInfo.salesPersonProfiles?.map((res) => {
              return {
                userId: res.UserID,
                userName: res.FirstName+' '+res.LastName,
                profileImg: res.salesPersonProfile
              }
            });
            this.salesPersonsList = this.salesPersonData;
            this.salesPersonNames = this.salesPersonsList.map((item) => item.userName);
            if (this.salesPersonsList.length > 0) {
              this.salesPersonsList.forEach((item) => this.salesPersonSelectedIds.push(item.userId));
            }
            this.threadInfo.salesPerson = "";
          }
          this.firstTime = 1;
        }, error => {
          this.manualsData = [];
  ...
  setFormValidation(flag, field) {
    if(flag) {
      ...
      } else if (field == 'maxParticipants' && this.threadInfo && this.signedupUsers && this.currentTrainingMode != 'Recorded') {
        this.threadForm.controls[field].setValidators([Validators.required]);
        this.customValidationMsg = "Max Participants should not less than registered users " + this.signedupUsers;
      } else {
        this.threadForm.controls[field].setValidators([Validators.required]);
      }
      this.threadForm.controls['zoomEmail']?.setValidators([Validators.required, Validators.pattern("^[a-zA0-9._%+-]+@[a-zA0-9.-]+\\.[a-z]{2,4}$")]);
    }
  }
  ...
  backStep1() {
    ...
    this.updateFreePrice(this.currentPaymentStatus);
    this.showBirdValues = threadFormData.isBirdPrice;
    this.birdValue = threadFormData.birdPrice;
    this.birdPercentage = threadFormData.birdPercentage;
    this.updateIsBirdPriceValue(this.showBirdValues);
  ...
  async onSubmit() {
    ...
    for(let f of this.formFields[this.stepIndex][this.stepTxt]) {
      ...
      if (!this.showBirdValues && (f.apiFieldKey == 'birdPrice' || f.apiFieldKey == 'birdPercentage' || f.apiFieldKey == 'expiryDateBirdPrice')) {
        f.valid = true;
      }
      if (this.showBirdValues && f.apiFieldKey == 'expiryDateBirdPrice' && this.threadForm.get('expiryDateBirdPrice').value) {
        f.valid = true;
      } else if (this.showBirdValues && f.apiFieldKey == 'expiryDateBirdPrice') {
        f.valid = false;
      }
      if (this.showBirdValues && f.apiFieldKey == 'birdPrice' && this.threadForm.get('birdPrice').value) {
        f.valid = true;
      } else if (this.showBirdValues && f.apiFieldKey == 'birdPrice') {
        f.valid = false;
      }
      if (this.showBirdValues && f.apiFieldKey == 'birdPercentage' && this.threadForm.get('birdPercentage').value) {
        f.valid = true;
      } else if (this.showBirdValues && f.apiFieldKey == 'birdPercentage') {
        f.valid = false;
      }
      if (this.currentTrainingMode == 'Recorded' && f.apiFieldKey == 'expiryDateBirdPrice') {
        f.valid = true;
      }
      ...
      if (['startDate', 'endDate', 'startTime', 'endTime', 'timeZone', 'hours', 'maxParticipants'].find(val => val == f.apiFieldKey)) {
        f.valid = this.threadForm.controls[f.apiFieldKey].errors ? false : true;
      }
      if (['sku', 'trainingMode', 'trainingName'].includes(f.apiFieldKey) &&  this.threadForm.getRawValue()[f.apiFieldKey]) {
        f.valid = !(this.threadForm.get(f.apiFieldKey).valid && this.sameName[f.apiFieldKey]);
        let noChanges = 0;
        if (this.threadId > 0) {
          ['sku', 'trainingMode', 'trainingName'].forEach((key) => {
            if (this.threadInfo[key] == this.threadForm.getRawValue()[key]) {
              noChanges++;
            }
          });
        }
    ...
    if(this.step1Submitted && !this.step2Submitted) {
      ...
      this.formFields[0]['step1'].forEach((field: any) => {
        ...
        if (this.currentTrainingMode == 'Webinar' && field.apiFieldKey != 'isFree' && field.apiFieldKey != 'venueName' && field.apiFieldKey != 'venueLink' && field.apiFieldKey != 'addressLine1' && field.apiFieldKey != 'addressLine2' && field.apiFieldKey != 'city' && field.apiFieldKey != 'state' && field.apiFieldKey != 'country' && field.apiFieldKey != 'zipCode' && field.apiFieldKey != 'hours' && !field.apiFieldKey.includes('inPersonNotes') && !field.apiFieldKey.includes('inPersonTimeZone') && !field.apiFieldKey.includes('inPersonEndTime') && !field.apiFieldKey.includes('inPersonDay') && !field.apiFieldKey.includes('inPersonStartTime') && field.apiFieldKey != 'recordedVideoLink' && field.apiFieldKey != 'birdPrice' && field.apiFieldKey != 'birdPercentage' && field.apiFieldKey != 'expiryDateBirdPrice' && field.apiFieldKey != 'isBirdPrice') {
          ...
          this.showAbleformFields.push(field);
        } else if (this.currentTrainingMode == 'Recorded' && field.apiFieldKey != 'maxParticipants' && field.apiFieldKey != 'isFree' && field.apiFieldKey != 'venueName' && field.apiFieldKey != 'venueLink' && field.apiFieldKey != 'addressLine1' && field.apiFieldKey != 'addressLine2' && field.apiFieldKey != 'city' && field.apiFieldKey != 'state' && field.apiFieldKey != 'country' && field.apiFieldKey != 'zipCode' && field.apiFieldKey != 'startDate' && field.apiFieldKey != 'endDate' && field.apiFieldKey != 'startTime' && field.apiFieldKey != 'endTime' && field.apiFieldKey != 'timeZone' && !field.apiFieldKey.includes('inPersonNotes') && !field.apiFieldKey.includes('inPersonTimeZone') && !field.apiFieldKey.includes('inPersonEndTime') && !field.apiFieldKey.includes('inPersonDay') && !field.apiFieldKey.includes('inPersonStartTime') && field.apiFieldKey != 'birdPrice' && field.apiFieldKey != 'birdPercentage' && field.apiFieldKey != 'expiryDateBirdPrice' && field.apiFieldKey != 'isBirdPrice') {
          if (field.apiFieldKey != 'workstream') {
            field.selectedValues = this.threadForm.get(field.apiFieldKey).value;
            field.selectedVal = this.threadForm.get(field.apiFieldKey).value;
            field.dateVal = this.threadForm.get(field.apiFieldKey).value;
        ...
        let threadFormData: any = this.threadForm.value;
        this.showBirdValues = threadFormData.isBirdPrice;
        if (this.showBirdValues && (field.apiFieldKey == 'birdPrice' || field.apiFieldKey == 'birdPercentage' || field.apiFieldKey == 'expiryDateBirdPrice')) {
          field.selectedValues = this.threadForm.get(field.apiFieldKey).value;
          field.selectedVal = this.threadForm.get(field.apiFieldKey).value;
          field.dateVal = this.threadForm.get(field.apiFieldKey).value;
          field.formValue = this.threadForm.get(field.apiFieldKey).value;
          if (this.currentTrainingMode == 'Recorded' && field.apiFieldKey == 'expiryDateBirdPrice') {
          } else {
            this.showAbleformFields.push(field);
          }
        }
  ...
  threadSubmit() {
    ...
    let startDate = threadFormData.get('startDate') ? moment(threadFormData.get('startDate')).format('YYYY-MM-DD') : null;
    let endDate = threadFormData.get('endDate') ? moment(threadFormData.get('endDate')).format('YYYY-MM-DD') : null;
    let startTime = threadFormData.get('startTime') ? moment(threadFormData.get('startTime')).format('YYYY-MM-DD HH:mm') : null;
    let endTime = threadFormData.get('endTime') ? moment(threadFormData.get('endTime')).format('YYYY-MM-DD HH:mm') : null;
    let expiryDateBirdPrice = threadFormData.get('expiryDateBirdPrice');
    let convertedExpiryDateBirdPrice = expiryDateBirdPrice && expiryDateBirdPrice != ' ' ? moment(threadFormData.get('expiryDateBirdPrice')).format('YYYY-MM-DD') : null
    threadFormData.set('startDate', startDate);
    threadFormData.set('endDate', endDate);
    threadFormData.set('startTime', startTime);
    threadFormData.set('endTime', endTime);
    threadFormData.set('expiryDateBirdPrice', convertedExpiryDateBirdPrice);
    if (this.currentTrainingMode == 'Recorded') {
      threadFormData.set('maxParticipants', null);
    }
    ...
    if(this.threadId > 0 && !this.duplicateMarketPlace) {
      ...
      setTimeout(() => {
        if (this.selectedMarketPlaceBanner) {
          ...
          //   this.marketPlaceBannerImgUrlServer = res.profile_path;
            ...
            threadFormData.set("salesPerson", JSON.stringify(this.salesPersonSelectedIds));
            threadFormData.set('manualId', this.manualId);
            if(this.manualId && !this.manualDiscountPrice) {
              threadFormData.set('isManualDiscountPrice', 0);
              threadFormData.set('manualDiscountPrice', (this.trainingManual.discountPrice && this.trainingManual.discountPrice != '0' ? this.trainingManual.discountPrice : ((this.trainingManual.price && this.trainingManual.price != '0') ? this.trainingManual.price : null)));
            } else {
              threadFormData.set('isManualDiscountPrice', 1);
              threadFormData.set('manualDiscountPrice', this.manualDiscountPrice);
            }
            threadFormData.set('manualPurchase', this.isManualIncludedWithTraining);
            this.threadApi.updateMarketPlace(threadFormData).subscribe((response) => {
              modalRef.dismiss('Cross click');
              ...
              if(msgFlag) {
                ...
                setTimeout(() => {
                  ...
                  localStorage.removeItem('marketNav');
                  this.router.navigateByUrl('market-place/view/'+ this.threadId);
                }, 2000);
              }
            });
          // }, (error: any) => {
      ...
      setTimeout(() => {
        if (this.duplicateMarketPlace) {
          ...
          threadFormData.set("mediaAttachments", JSON.stringify(this.mediaUploadItems));
          threadFormData.set('duplicateId', this.threadId);
        }
        if (this.selectedMarketPlaceBanner) {
          let serverUrl = this.apiUrl.apifileUpload();
          let bannerFormData = new FormData();
          ...
          //   this.marketPlaceBannerImgUrlServer = res.profile_path;
            ...
            this.threadApi.createMarketPlace(threadFormData).subscribe((response) => {
              modalRef.dismiss('Cross click');
              this.bodyElem.classList.remove(this.bodyClass);
              this.successMsg = response.message;
  ...
  updateFreePrice(event: any) {
    ...
    if (event) {
      ...
      this.threadForm.controls['isBirdPrice'].disable();
    } else {
      this.threadForm.patchValue({
        price: null,
        isBirdPrice: false,
      ...
      this.threadForm.controls['isBirdPrice'].enable();
    }
    if (!this.stepBack) {
  ...
  updateStartDateSteps(event: any) {
    if (this.endDate < event) {
      ...
      this.miniumExpiryDateBird = event;
      if (!this.endDate) {
        this.threadForm.patchValue({
          endDate: event
        })
  ...
  hideShowDateFields() {
    if(['Seminar', 'Webinar'].includes(this.currentTrainingMode) && this.endDate){
      var maxDate = new Date(this.startDate);
      maxDate.setDate(maxDate.getDate() + 6);
      if (this.endDate > maxDate) {
        this.endDate = maxDate;
        this.threadForm.get('endDate').setValue(maxDate)
      }
    }
    if (this.startDate && this.endDate && this.currentTrainingMode == 'Seminar') {
      for(let i = 0; i < 7; i++) {
        this.formFields[this.stepIndex][this.stepTxt].forEach((fields: any) => {
          if (fields.apiFieldKey == 'inPersonDay'+i && !this.threadForm.get('inPersonDay'+i).value) {
            fields.valid = false;
          }
          if (fields.apiFieldKey == 'inPersonStartTime'+i && !this.threadForm.get('inPersonStartTime'+i).value) {
            fields.valid = false;
          }
          if (fields.apiFieldKey == 'inPersonEndTime'+i && !this.threadForm.get('inPersonEndTime'+i).value) {
            fields.valid = false;
          }
          if (fields.apiFieldKey == 'inPersonTimeZone'+i && !this.threadForm.get('inPersonTimeZone'+i).value) {
            fields.valid = false;
          }
          if (this.resetStartEndDate) {
            if (fields.apiFieldKey == 'inPersonStartTime'+i) {
              fields.valid = false;
            }
            if (fields.apiFieldKey == 'inPersonEndTime'+i) {
              fields.valid = false;
            }
            if (fields.apiFieldKey == 'inPersonTimeZone'+i) {
              fields.valid = false;
            }
          }
        });
        this.apiFormFields[this.stepIndex][this.stepTxt].forEach((form: any) => {
          if(form.stepsName.includes('In-person time select')) {
            form.newSectionClass = 'hide';
            form.newCustomLabel = '';
            form.newCustomClass = '';
          }
          form?.cells?.name?.forEach((field: any) => {
            if (field.apiFieldKey == 'inPersonDay'+i && !this.threadForm.get('inPersonDay'+i).value) {
              field.valid = false;
            }
            if (field.apiFieldKey == 'inPersonStartTime'+i && !this.threadForm.get('inPersonStartTime'+i).value) {
              field.valid = false;
            }
            if (field.apiFieldKey == 'inPersonEndTime'+i && !this.threadForm.get('inPersonEndTime'+i).value) {
              field.valid = false;
            }
            if (field.apiFieldKey == 'inPersonTimeZone'+i && !this.threadForm.get('inPersonTimeZone'+i).value) {
              field.valid = false;
            }
            if (this.resetStartEndDate) {
              if (field.apiFieldKey == 'inPersonStartTime'+i) {
                field.dateVal = '';
                field.valid = false;
              }
              if (field.apiFieldKey == 'inPersonEndTime'+i) {
                field.dateVal = '';
                field.valid = false;
              }
              if (field.apiFieldKey == 'inPersonTimeZone'+i) {
                field.selectedValues = null;
                field.valid = false;
              }
              if (field.apiFieldKey == 'inPersonNotes'+i) {
      ...
      for(let i = 0; i <= diffDays; i++) {
        ...
        let getDayFullName = moment(date).format('dddd');
        this.apiFormFields[this.stepIndex][this.stepTxt].forEach((formArray: any, formArrayIndex: any) => {
          if(formArray.stepsName == 'In-person time select ' + i) {
            formArray.newSectionClass = 'show';
            formArray.newCustomLabel = getDayFullName+', '+ this.getDateFormat(date);
            formArray.newCustomClass = 'label-custom-design calendor-icon';
          }
        });
        let field: any = 'inPersonDay'+i;
        let getDayName = moment(date).format('ddd');
        let setDayValue: any = {};
        setDayValue[field] = getDayName;
        this.threadForm.patchValue(setDayValue);
        this.threadForm.controls[field].disable();
        this.formFields[this.stepIndex][this.stepTxt].forEach((fields: any) => {
          if (fields.apiFieldKey == 'inPersonDay'+i) {
            fields.vaild = true;
          }
        })
      }
      let validDiffDays = diffDays + 1;
      for(let i = validDiffDays; i < 7; i++) {
        this.formFields[this.stepIndex][this.stepTxt].forEach((fields: any) => {
          if (fields.apiFieldKey == 'inPersonDay'+i) {
            fields.valid = true;
          }
        ...
        this.apiFormFields[this.stepIndex][this.stepTxt].forEach((form: any) => {
          if(form.stepsName == 'In-person time select ' + i) {
            form.newSectionClass = 'hide';
          }
          form?.cells?.name?.forEach((field: any) => {
            if (field.apiFieldKey == 'inPersonDay'+i) {
              field.valid = true;
            }
            if (field.apiFieldKey == 'inPersonStartTime'+i) {
              field.valid = true;
  ...
  updateTrainingModeSteps(event: any) {
    console.log(event, this.currentTrainingMode);
    if (this.startDate && this.endDate && this.currentTrainingMode == 'Seminar' && this.firstTime && this.step1) {
      this.threadForm.patchValue({
        trainingMode: this.currentTrainingMode
      });
      setTimeout(() => {
        const modalRef = this.modalService.open(ConfirmationComponent, this.modalConfig);
        modalRef.componentInstance.access = 'changeTrainingMode';
        modalRef.componentInstance.confirmAction.subscribe((receivedService) => {
          modalRef.dismiss('Cross click');
          if(!receivedService) {
            this.threadForm.patchValue({
            ...
            this.currentTrainingMode = this.threadForm.get('trainingMode').value;
            this.trainingModeFieldHideShow();
          } else {
            this.currentTrainingMode = event.val;
            this.trainingModeFieldHideShow();
            this.trainingModeChanged = true;
          }
        });
      }, 500);
    } else {
      this.currentTrainingMode = event.val;
      this.threadForm.get('trainingMode').setValue(
        this.currentTrainingMode
      );
      this.trainingModeFieldHideShow();
    }

  }
  trainingModeFieldHideShow() {
    if (this.currentTrainingMode == 'Webinar') {
      this.showOnlineField = true;
      this.showHoursField = false;
      this.showInPersonField = false;
      this.threadForm.patchValue({
        maxParticipants: null,
        expiryDateBirdPrice: null,
      });
      this.threadForm.controls['maxParticipants'].enable();
      this.threadForm.controls['expiryDateBirdPrice'].enable();
      this.threadForm.get('expiryDateBirdPrice').setValidators([Validators.required]);
      this.threadForm.get('startDate').setValidators([Validators.required]);
      this.threadForm.get('endDate').setValidators([Validators.required]);
      this.threadForm.get('startTime').setValidators([Validators.required]);
      this.threadForm.get('endTime').setValidators([Validators.required]);
      this.threadForm.get('timeZone').setValidators([Validators.required]);
      this.threadForm.get('maxParticipants').setValidators([Validators.required]);
      this.threadForm.get('recordedVideoLink').clearValidators();
      this.threadForm.get('expiryDateBirdPrice').updateValueAndValidity();
      this.threadForm.get('hours').clearValidators();
      this.threadForm.get('addressLine1').clearValidators();
      this.threadForm.get('addressLine2').clearValidators();
      this.threadForm.get('venueName').clearValidators();
      this.threadForm.get('venueLink').clearValidators();
      this.threadForm.get('city').clearValidators();
      this.threadForm.get('state').clearValidators();
      this.threadForm.get('country').clearValidators();
      this.threadForm.get('zipCode').clearValidators();
      this.threadForm.get('startDate').updateValueAndValidity();
      this.threadForm.get('endDate').updateValueAndValidity();
      this.threadForm.get('startTime').updateValueAndValidity();
      this.threadForm.get('endTime').updateValueAndValidity();
      this.threadForm.get('timeZone').updateValueAndValidity();
      this.threadForm.get('hours').updateValueAndValidity();
      this.threadForm.get('addressLine1').updateValueAndValidity();
      this.threadForm.get('addressLine2').updateValueAndValidity();
      this.threadForm.get('venueName').updateValueAndValidity();
      this.threadForm.get('venueLink').updateValueAndValidity();
      this.threadForm.get('city').updateValueAndValidity();
      this.threadForm.get('recordedVideoLink').updateValueAndValidity();
      this.threadForm.get('state').updateValueAndValidity();
      this.threadForm.get('country').updateValueAndValidity();
      this.threadForm.get('zipCode').updateValueAndValidity();
      this.threadForm.get('maxParticipants').updateValueAndValidity();
    } else if (this.currentTrainingMode == 'Seminar') {
      this.showOnlineField = false;
      this.showHoursField = false;
      this.showInPersonField = true;
      this.threadForm.patchValue({
        maxParticipants: null,
        expiryDateBirdPrice: null,
      });
      ...
      this.threadForm.get('endDate').setValidators([Validators.required]);
      this.threadForm.get('recordedVideoLink').clearValidators();
      this.threadForm.get('startTime').clearValidators();
      this.threadForm.get('expiryDateBirdPrice').setValidators([Validators.required]);
      this.threadForm.get('endTime').clearValidators();
      this.threadForm.get('expiryDateBirdPrice').updateValueAndValidity();
      this.threadForm.get('timeZone').clearValidators();
      this.threadForm.get('hours').clearValidators();
      this.threadForm.get('addressLine1').setValidators([Validators.required]);
      this.threadForm.get('addressLine2').setValidators([Validators.required]);
      this.threadForm.get('venueName').setValidators([Validators.required]);
      this.threadForm.get('venueLink').clearValidators();
      this.threadForm.get('maxParticipants').setValidators([Validators.required]);
      this.threadForm.get('city').setValidators([Validators.required]);
      this.threadForm.get('state').setValidators([Validators.required]);
      this.threadForm.get('country').setValidators([Validators.required]);
      this.threadForm.get('zipCode').setValidators([Validators.required]);
      this.threadForm.get('startDate').updateValueAndValidity();
      this.threadForm.get('endDate').updateValueAndValidity();
      this.threadForm.get('startTime').updateValueAndValidity();
      this.threadForm.get('endTime').updateValueAndValidity();
      ...
      this.threadForm.get('recordedVideoLink').updateValueAndValidity();
      this.threadForm.get('maxParticipants').updateValueAndValidity();
    } else if (this.currentTrainingMode == 'Recorded') {
      this.showOnlineField = false;
      this.showHoursField = true;
      this.showInPersonField = false;
      this.threadForm.controls['maxParticipants'].disable();
      this.threadForm.patchValue({
        maxParticipants: 'Max participants',
        expiryDateBirdPrice: ' ',
      });
      this.threadForm.controls['expiryDateBirdPrice'].disable();
      this.threadForm.get('startDate').clearValidators();
      this.threadForm.get('expiryDateBirdPrice').clearValidators();
      this.threadForm.get('endDate').clearValidators();
      this.threadForm.get('expiryDateBirdPrice').updateValueAndValidity();
      this.threadForm.get('startTime').clearValidators();
      this.threadForm.get('endTime').clearValidators();
      this.threadForm.get('timeZone').clearValidators();
      this.threadForm.get('maxParticipants').clearValidators();
      this.threadForm.get('hours').setValidators([Validators.required]);
      this.threadForm.get('recordedVideoLink').setValidators([Validators.required]);
      this.threadForm.get('addressLine1').clearValidators();
      this.threadForm.get('addressLine2').clearValidators();
      this.threadForm.get('venueName').clearValidators();
      this.threadForm.get('venueLink').clearValidators();
  ...
  updateMaxParticipantValidation(event: any) {
    if (parseInt(event) < this.signedupUsers && this.currentTrainingMode != 'Recorded') {
    ...
    if (event) {
      this.apiFormFields[this.stepIndex][this.stepTxt].map((form: any) => {
        form?.cells?.name?.map((field: any) => {
          if (field.apiFieldKey == 'birdPrice') {
            field.fieldClass = 'birdPrice col-md-4 show'
          }
          if (field.apiFieldKey == 'birdPercentage') {
            field.fieldClass = 'birdPercentage col-md-4 show'
          }
          if (field.apiFieldKey == 'expiryDateBirdPrice') {
            field.fieldClass = 'expiryDateBirdPrice col-md-4 show'
          }
        })
      });
    } else {
      this.apiFormFields[this.stepIndex][this.stepTxt].map((form: any) => {
        form?.cells?.name?.map((field: any) => {
          if (field.apiFieldKey == 'birdPrice') {
            field.fieldClass = 'birdPrice col-md-4'
          }
          if (field.apiFieldKey == 'birdPercentage') {
            field.fieldClass = 'birdPercentage col-md-4'
          }
          if (field.apiFieldKey == 'expiryDateBirdPrice') {
            field.fieldClass = 'expiryDateBirdPrice col-md-4'
          }
        })
      });
      if (!this.stepBack) {
        this.birdPercentage = '';
        this.birdValue = '';
        this.threadForm.get('expiryDateBirdPrice').setValue('');
      }
    }
  }

  updateRegularPrice(event: any) {
    this.updateBirdPriceValue(this.birdValue);
    if (event) {
      this.threadForm.controls['birdPrice'].enable();
      this.threadForm.controls['birdPercentage'].enable();
    }
  }
  ...
  updateBirdPriceValue(event: any){
    ...
    if (!price) {
      this.threadForm.controls['birdPrice'].disable();
      this.threadForm.controls['birdPercentage'].disable();
      return
    }
    if (event && price) {
  ...
  updateBirdPercentageValue(event: any) {
    ...
    if (event && price) {
      ...
      if (perc < 100 && !this.isNegative(originalPerc) && perc != '0') {
        ...
        this.birdPercentage = event;
        let val: any = (parseFloat(priceFloat) * parseFloat(originalPerc))/100;
        this.birdValue = parseFloat(val).toFixed(0);
        this.showBirdPriceValidation = false;
        this.showBirdPercentageValidation = false;
      } else {
        this.birdPercentage = '';
  ...
  showSalesPersonPopupFn(event){
    this.workStreamIds = this.threadForm?.controls?.workstream?.value?.map(element => element.id);
    this.openSalesPersonPopup = event;
    // console.log(event);
  }

  getSalesPerson(event){
    this.salesPersonsList = event;
    this.salesPersonSelectedIds = [];
    this.salesPersonsList.forEach((item) => this.salesPersonSelectedIds.push(item.userId));
    this.salesPersonNames = this.salesPersonsList.map((item) => item.userName);
    this.openSalesPersonPopup = false;
  }

  removeSalesPerson(event) {
    let index = this.salesPersonsList.indexOf(event);
    if (index !== -1) {
      this.salesPersonsList.splice(index, 1);
    }
    this.salesPersonSelectedIds = [];
    this.salesPersonsList.forEach((item) => this.salesPersonSelectedIds.push(item.userId));
    this.openSalesPersonPopup = false;
  ...
  updateManualIncludedWithTraining(event){
    this.isManualIncludedWithTraining = event;
  }
  ...
  trainingsTitleListPass(event) {
    this.trainingsTitleListGet = event;
  }
