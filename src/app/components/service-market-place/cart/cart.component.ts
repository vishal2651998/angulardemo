import { Component, OnInit, ViewChild, ElementRef, Renderer2, Inject, DoCheck, ChangeDetectorRef } from "@angular/core";
...
export class CartComponent implements OnInit {
  ...
  pageAccess: string = 'market-place-cart';
  ...
  ngOnInit() {
    ...
    this.cartFilled = localStorage.getItem('adminCartId');
    this.getCart(true);
  ...
  get alternatePricingFormControl() {
    return this.alternatePricingForm.controls;
  }
  ...
  getCart(showLoader = false) {
    ...
    if (cartId) this.threadApi.getCart({ cartId: cartId }).pipe(debounceTime(100)).subscribe((resp) => {
    ...
    switch (tab) {
      ...
      case "manuals":
        if (((noParticipant || this.companyForm.invalid || this.userForm.invalid) && this.cart.trainings.length > 0 ) || this.cartForm.invalid) {
          ...
          if((this.companyForm.invalid || this.userForm.invalid) && this.cart.trainings.length == 0){
          this.userFormSubmitted= true;
          this.companyFormSubmitted = true;
          } else {
            this.selectedInfoType = "shippingInfo";
      ...
      case "summary":
        this.changeInfoType('paymentDetails')
        break;
    }
    this.sidebarHandler();
  }
  ...
  setCart(data) {
    this.cart = {
      cartId: data?.id,
      userId: data?.userId,
      userDetail: data?.userDetail ? { ...data?.userDetail, isMemberSelected: true } : null,
      email: data?.email,
      ...
      data.trainings.forEach((training, index) => {
        ...
        if (!this.cart?.cartForm?.trainings.map((tr) => tr.trainingId).includes(training.id)) {
          this.cartTrainings.push(this.fb.group({
            ...
            manualoverRidePriceReason:  null,
            manualOverRidePercent:  null,
            manualIsOverRidePrice: false,
          }))
        ...
        /* push training if value exists in form */
          ...
          this.cartTrainings.push(this.fb.group({
            trainingId: training.id,
            overRidePrice: currTraining.overRidePrice || null,
            overRidePriceReason: currTraining.overRidePriceReason || null,
            overRidePercent: currTraining.overRidePercent || null,
            isOverRidePrice: currTraining.isOverRidePrice  || false,
            participants: this.fb.array([]),
            manualCount: currTraining.manualCount || 0,
            manualOverRidePrice: currTraining.manualOverRidePrice || null,
            manualoverRidePriceReason: currTraining.manualoverRidePriceReason || null,
            manualOverRidePercent: currTraining.manualOverRidePercent || null,
            manualIsOverRidePrice: currTraining.manualIsOverRidePrice  || false,
          }));
    ...
    if (data.manuals?.length == 0) this.infoType[1].isVisible = false;
      ...
      data.manuals.forEach((manual) => {
        ...
        /* push manual if exists in cartForm */
          ...
          this.cartManuals.push(this.fb.group({
            overRidePrice: currManual.overRidePrice || null,
            overRidePercent: currManual.overRidePercent || null,
            overRidePriceReason: currManual.overRidePriceReason || null,
            isOverRidePrice: currManual.isOverRidePrice  || false,
            id: manual.id,
    ...
    this.showLoader = false;
    if (!(this.cart?.trainings?.length > 0 || this.cart?.manuals?.length > 0)) {
      this.emptyCartPopup = true;
      this.cartFilled = '';
    }
  ...
  trainingsPrice(trainingIndex: number) {
    return this.cartTrainings.at(trainingIndex).value
  }

  manualPrice(manualIndex: number) {
    if (this.cartManuals.at(manualIndex)) {
      return this.cartManuals.at(manualIndex).value;
    } /* else if ((manualIndex + 1) > this.cartManuals.length) {
      let trData = this.cart?.trainings.filter((tr, index) =>
        tr.manual && this.getTrainingManualCount(index).value && this.getTrainingManualCount(index).value != 0);
      let curIndex = this.cartManuals.length - (manualIndex - 1);
      let trIndex = this.cart?.trainings.findIndex((res) => res.id == trData[curIndex].id)
      return this.cartTrainings.at(trIndex).value;
    } */ else {
      return false;
    }
  }
  ...
  checkSimilarEmail(trainingIndex,participantIndex) {
    if (this.cart.trainings[trainingIndex].trainingMode == 'Webinar') {
      let index = this.participants(trainingIndex).value.findIndex((res,index)=>index !=participantIndex && res.email == this.participants(trainingIndex).at(participantIndex).get('email').value)
      let prevIndex = this.cart.trainings[trainingIndex].registeredUsers.findIndex((res,index)=>res == this.participants(trainingIndex).at(participantIndex).get('email').value)
    ...
    this.filteredParticipants.forEach((item) => {
     ...
     this.participants(this.selectedTrainingIndex).push(this.fb.group({
        ...
        if (this.cart.trainings[this.selectedTrainingIndex].trainingMode == 'Webinar') {
          ...
          this.customValidator = false;
          this.checkSimilarEmail(this.selectedTrainingIndex,this.participants(this.selectedTrainingIndex).length - 1)
        }
      }
    })
  ...
  totalManuals() {
    ...
    if (this.cart?.manuals) this.cart?.manuals.forEach((manual, index) => {
      let  mlData = this.manualPrice(index);
      if (mlData) {
        manuals.push({ ...manual, count: mlData.count, overRidePrice: mlData.overRidePrice, overRidePercent: mlData.overRidePercent, isOverRidePrice: mlData.isOverRidePrice });
      }
    })
    if (this.cart?.trainings) this.cart?.trainings.forEach((tr, index) => {
      if (tr.manual && this.getTrainingManualCount(index).value && this.getTrainingManualCount(index).value != 0) {
      let  training = this.trainingsPrice(index);
        manuals.push({ ...tr.manual, trIndex:index, trainingManualInfo: tr?.manualPurchase == '2' ? 'Included Manual with Training' : 'Recomended Manual for Training',
        count: this.getTrainingManualCount(index).value , overRidePrice: training.manualOverRidePrice , overRidePriceReason: training.manualoverRidePriceReason, overRidePercent: training.manualOverRidePercent, isOverRidePrice: training.manualIsOverRidePrice});
      }
    })
    return manuals;
  }
  ...
  getTotalPrice(type, roundOff = false, addDollar = true) {
    ...
    if (['training','trainingsSales','trainingsSalesWithoutTax','all'].includes(type)) {
      this.cart?.trainings?.forEach((tr, index) => {
        trainingPrice += this.participants(index).controls.length * parseFloat(this.trainingsPrice(index).isOverRidePrice ? (this.trainingsPrice(index).overRidePrice || 0) : tr.isBirdPrice == '1' ? tr.birdPrice : tr.price);
      })
    ...
    if (['manual', 'manualsSales','manualsSalesWithoutTax','all'].includes(type)) {
      this.totalManuals().forEach((manual, index) => {
        manualPrice += manual.count * parseFloat(( manual)?.isOverRidePrice ? (manual.overRidePrice || 0) : manual.isDiscount == '1' ? manual.discountPrice : manual.price);
    ...
    this.cart.trainings.forEach((tr, ti) => {
      ...
      if (tr.manual) {
        ...
        if (tr?.manualPurchase == '2') {
          ...
          let paidPrice = trData?.manualIsOverRidePrice ? trData?.manualOverRidePrice : tr.isManualDiscountPrice == '1' ? tr.manualDiscountPrice : (tr.manual.discountPrice && tr.manual.discountPrice != '0') ? tr.manual.discountPrice : tr.manual.price;
          ...
          tr.manual = {
            ...
            taxResponse: this.taxResponse, shippingCost: (this.defaultShippingCost || 0), overRidePrice: trData?.manualOverRidePrice, overRidePriceReason: trData?.manualoverRidePriceReason, overRidePercent: trData?.manualOverRidePercent, isOverRidePrice: trData?.manualIsOverRidePrice
          }
        }
      }
      let paidPrice = trData.isOverRidePrice ? trData.overRidePrice : (tr.birdPrice && tr.birdPrice != '0') ? tr.birdPrice : tr.price;
      ...
      trainings.push({
        ...
        taxResponse: this.taxResponse, overRidePrice: trData.overRidePrice,overRidePriceReason: trData.overRidePriceReason , overRidePercent: trData.overRidePercent, isOverRidePrice: trData.isOverRidePrice
      });
    })
    this.cart.manuals.forEach((ml, mi) => {
      let mlData = (this.cartForm.value).manuals.find(element => element.id == ml.id);
      if (mlData) {
        let paidPrice = mlData.isOverRidePrice ? mlData.overRidePrice : (ml.discountPrice && ml.discountPrice != '0') ? ml.discountPrice : ml.price;
        ...
        manuals.push({
          ...
          paidPrice: paidPrice, actualPrice: actualPrice,
          salesTax: (paidPrice && paidPrice == '0') ? 0 : salesTax, salesTaxPercent: (paidPrice && paidPrice == '0') ? 0 : this.cart?.manualSalesTaxPercentage || 0,
          taxResponse: this.taxResponse, shippingCost: (this.defaultShippingCost || 0), overRidePrice: mlData.overRidePrice, overRidePriceReason: mlData.overRidePriceReason, overRidePercent: mlData.overRidePercent, isOverRidePrice: mlData.isOverRidePrice
        })
      }
    })
    ...
    if (this.cart.trainings.length > 0) {
      ...
      if (saveFrom == 'userForm' && user.sameAsCheck.value) {
        if (this.cart.trainings[0]?.trainingMode == 'Webinar') {
          let prevEmails = this.cart.trainings[0].registeredUsers.findIndex((res)=> res == user.email.value);
         if (prevEmails != -1) {
          firstParticipant.get('email').setValue(null);
  ...
  showPricingPopup(trainingIndex = null, manualIndex = null, isTrainingManual = false) {
    this.overRidePriceTrainingIndex = trainingIndex;
    this.overRidePriceManualIndex = manualIndex;
    this.isTrainingManualOverride = isTrainingManual;
    this.freeOffer = false;
    this.alternatePricing = false;
    this.pricingFormSubmitted = false;
    /* training */
    if (!this.isTrainingManualOverride && this.overRidePriceTrainingIndex != null) {
      let currentTraining = this.cartTrainings.at(this.overRidePriceTrainingIndex);
      this.birdValue = currentTraining.get('overRidePrice').value;
      this.birdPercentage = currentTraining.get('overRidePercent').value;
      this.birdValueReason = currentTraining.get('overRidePriceReason').value;
      this.overRideDetails = {
        type: 'training',
        name: this.cart?.trainings[this.overRidePriceTrainingIndex]?.trainingName,
        regular: ((!this.cart?.trainings[this.overRidePriceTrainingIndex]?.price) ||
          (this.cart?.trainings[this.overRidePriceTrainingIndex]?.price == '0')) ? 'Free' :
          this.threadApi.isInt(this.cart?.trainings[this.overRidePriceTrainingIndex]?.price, true, true),
        discount: this.cart?.trainings[this.overRidePriceTrainingIndex]?.birdPrice ? this.threadApi.isInt(this.cart?.trainings[this.overRidePriceTrainingIndex]?.birdPrice, true, true) : '-',
        discountPercent: this.cart?.trainings[this.overRidePriceTrainingIndex]?.birdPercentage ? this.threadApi.isInt(this.cart?.trainings[this.overRidePriceTrainingIndex]?.birdPercentage, false, true) + '%' : '-',
      }
      if (currentTraining.get('isOverRidePrice').value == true) {
        if (this.birdValue) {
          this.alternatePricing = true;
        }
        else {
          this.freeOffer = true;
        }
    ...
    if (this.isTrainingManualOverride && this.overRidePriceTrainingIndex != null) {
      let currentTraining = this.cartTrainings.at(this.overRidePriceTrainingIndex);
      this.birdValue = currentTraining.get('manualOverRidePrice').value;
      this.birdPercentage = currentTraining.get('manualOverRidePercent').value;
      this.birdValueReason = currentTraining.get('manualoverRidePriceReason').value;
      this.overRideDetails = {
        type: 'manual',
        name: this.cart?.trainings[this.overRidePriceTrainingIndex]?.manual.manualName,
        regular: ((!this.cart?.trainings[this.overRidePriceTrainingIndex]?.manual.price) || (this.cart?.trainings[this.overRidePriceTrainingIndex]?.manual.price == '0')) ? 'Free' : this.threadApi.isInt(this.cart?.trainings[this.overRidePriceTrainingIndex]?.manual.price, true, true),
        discount: this.cart?.trainings[this.overRidePriceTrainingIndex]?.manual.discountPrice ? this.threadApi.isInt(this.cart?.trainings[this.overRidePriceTrainingIndex]?.manual.discountPrice, true, true) : '-',
        discountPercent: this.cart?.trainings[this.overRidePriceTrainingIndex]?.manual.birdPercentage ? this.threadApi.isInt(this.cart?.trainings[this.overRidePriceTrainingIndex]?.manual.birdPercentage, false, true) + '%' : '-',

      }
      if(currentTraining.get('manualIsOverRidePrice').value == true) {
        if (this.birdValue) {
          this.alternatePricing = true;
        }
        else {
    ...
    /* manual */
    if (this.overRidePriceManualIndex != null) {
      let currentManual = this.cartManuals.at(this.overRidePriceManualIndex);
      this.birdValue = currentManual.get('overRidePrice').value;
      this.birdPercentage = currentManual.get('overRidePercent').value;
      this.birdValueReason = currentManual.get('overRidePriceReason').value;
      this.overRideDetails = {
        type: 'manual',
        name: this.cart?.manuals[this.overRidePriceManualIndex]?.manualName,
        regular: ((!this.cart?.manuals[this.overRidePriceManualIndex]?.price) || (this.cart?.manuals[this.overRidePriceManualIndex]?.price == '0')) ? 'Free' : this.threadApi.isInt(this.cart?.manuals[this.overRidePriceManualIndex]?.price, true, true),
        discount: this.cart?.manuals[this.overRidePriceManualIndex]?.discountPrice ? this.threadApi.isInt(this.cart?.manuals[this.overRidePriceManualIndex]?.discountPrice, true, true) : '-',
        discountPercent: this.cart?.manuals[this.overRidePriceManualIndex]?.birdPercentage ? this.threadApi.isInt(this.cart?.manuals[this.overRidePriceManualIndex]?.birdPercentage, false, true) + '%' : '-',
      }
      if(currentManual.get('isOverRidePrice').value == true) {
        if (this.birdValue) {
          this.alternatePricing = true;
        }
       else {
  ...
  closePricingPopup() {
    ...
    this.birdPercentage = '';
    this.birdValue = '';
    this.birdValueReason = '';
    this.isTrainingManualOverride = false;
  }
  ...
  updateIsBirdPriceValue() {
    ...
    if(this.alternatePricing) {
      this.alternatePricingForm.get('birdPrice').setValidators([Validators.required]);
      this.alternatePricingForm.get('birdPercentage').setValidators([Validators.required]);
      this.alternatePricingForm.get('birdValueReason').setValidators([Validators.required]);
      this.freeOffer = false ;
    } else {
      this.alternatePricingForm.reset();
      this.alternatePricingForm.get('birdPrice').clearValidators();
      this.alternatePricingForm.get('birdPercentage').clearValidators();
      this.alternatePricingForm.get('birdValueReason').clearValidators();
      // if (!this.isTrainingManualOverride && this.overRidePriceTrainingIndex != null) {
      //   this.payablePrice = this.cart?.trainings[this.overRidePriceTrainingIndex].birdPrice && this.cart?.trainings[this.overRidePriceTrainingIndex].birdPrice != '0' && this.checkBirdPriceAvailablity(this.cart?.trainings[this.overRidePriceTrainingIndex].expiryDateBirdPrice) ? this.cart?.trainings[this.overRidePriceTrainingIndex].birdPrice : (this.cart?.trainings[this.overRidePriceTrainingIndex].price && this.cart?.trainings[this.overRidePriceTrainingIndex].price != '0' ? this.cart?.trainings[this.overRidePriceTrainingIndex].price : 0);
      // }
      // if (this.isTrainingManualOverride && this.overRidePriceTrainingIndex != null) {
      //   this.payablePrice = (this.cart?.trainings[this.overRidePriceTrainingIndex].manual.birdPrice && this.cart?.trainings[this.overRidePriceTrainingIndex].manual.birdPrice != '0') ? this.cart?.trainings[this.overRidePriceTrainingIndex].manual.birdPrice : this.cart?.trainings[this.overRidePriceTrainingIndex].manual.price;
      // }
      // if (this.overRidePriceManualIndex != null) {
      //   this.payablePrice = (this.cart?.manuals[this.overRidePriceManualIndex].discountPrice && this.cart?.manuals[this.overRidePriceManualIndex].discountPrice != '0') ? this.cart?.manuals[this.overRidePriceManualIndex].discountPrice : this.cart?.manuals[this.overRidePriceManualIndex].price;
      // }
    }
    this.showBirdPercentageValidation = false;
    this.showBirdPriceValidation = false;
    this.alternatePricingForm.get('birdPrice').updateValueAndValidity();
    this.alternatePricingForm.get('birdPercentage').updateValueAndValidity();
  }
  ...
  updateFreeOfferValue() {
    ...
    //   }
    //   if (this.overRidePriceManualIndex != null) {
    //     this.payablePrice = (this.cart?.manuals[this.overRidePriceManualIndex].birdPrice && this.cart?.manuals[this.overRidePriceManualIndex].birdPrice != '0') ? this.cart?.manuals[this.overRidePriceManualIndex].birdPrice : this.cart?.manuals[this.overRidePriceManualIndex].price;
    //   }
    // }
  }
  ...
  updateBirdPriceValue(event: any){
    let price ;
    if (!this.isTrainingManualOverride && this.overRidePriceTrainingIndex != null) {
      if (this.cart?.trainings[this.overRidePriceTrainingIndex]?.overRidePrice) {
        price = this.cart?.trainings[this.overRidePriceTrainingIndex]?.overRidePrice
      }
      // else if(this.cart?.trainings[this.overRidePriceTrainingIndex]?.birdPrice && this.cart?.trainings[this.overRidePriceTrainingIndex]?.birdPrice != 0 ) {
      //   price = this.cart?.trainings[this.overRidePriceTrainingIndex]?.birdPrice
      // }
      else {
        price = this.cart?.trainings[this.overRidePriceTrainingIndex]?.price
      }
    }
    else if (this.isTrainingManualOverride && this.overRidePriceTrainingIndex != null) {
      // price = this.cart?.trainings[this.overRidePriceTrainingIndex]?.manual.overRidePrice ?
      //   this.cart?.trainings[this.overRidePriceTrainingIndex]?.manual.overRidePrice :
      //   (this.cart?.trainings[this.overRidePriceTrainingIndex]?.manual.discountPrice && this.cart?.trainings[this.overRidePriceTrainingIndex]?.manual.discountPrice != 0) ?
      //     this.cart?.trainings[this.overRidePriceTrainingIndex]?.manual.discountPrice : this.cart?.trainings[this.overRidePriceTrainingIndex]?.manual.price
      price = this.cart?.trainings[this.overRidePriceTrainingIndex]?.manual.overRidePrice ?
        this.cart?.trainings[this.overRidePriceTrainingIndex]?.manual.overRidePrice : this.cart?.trainings[this.overRidePriceTrainingIndex]?.manual.price
    }
    else {
      if (this.cart?.manuals[this.overRidePriceManualIndex]?.overRidePrice) {
        price = this.cart?.manuals[this.overRidePriceManualIndex]?.overRidePrice
      }
      // else if(this.cart?.manuals[this.overRidePriceManualIndex]?.discountPrice && this.cart?.manuals[this.overRidePriceManualIndex]?.discountPrice != 0 ) {
      //   price = this.cart?.manuals[this.overRidePriceManualIndex]?.discountPrice
      // }
      else {
        price = this.cart?.manuals[this.overRidePriceManualIndex]?.price
      }
    }
    if (event && price) {
  ...
  updateBirdPercentageValue(event: any) {
    let price;
    if (!this.isTrainingManualOverride && this.overRidePriceTrainingIndex != null) {
      price = this.cart?.trainings[this.overRidePriceTrainingIndex]?.isOverRidePrice ? this.cart?.trainings[this.overRidePriceTrainingIndex]?.overRidePrice : this.cart?.trainings[this.overRidePriceTrainingIndex]?.price
        // this.cart?.trainings[this.overRidePriceTrainingIndex]?.birdPrice || this.cart?.trainings[this.overRidePriceTrainingIndex]?.price
    }
    else if (this.isTrainingManualOverride && this.overRidePriceTrainingIndex != null) {
      // price = this.cart?.trainings[this.overRidePriceTrainingIndex]?.manual.overRidePrice ?
      //   this.cart?.trainings[this.overRidePriceTrainingIndex]?.manual.overRidePrice :
  ...
  updateAlternatePrice() {
    this.pricingFormSubmitted = false;
    if (!this.isTrainingManualOverride && this.overRidePriceTrainingIndex != null) {
      let currentTraining = this.cartTrainings.at(this.overRidePriceTrainingIndex);
      if (this.alternatePricing) {
        if (this.alternatePricingForm.valid) {
          currentTraining.get('overRidePrice').setValue(this.birdValue);
          currentTraining.get('overRidePercent').setValue(this.birdPercentage);
          currentTraining.get('isOverRidePrice').setValue(true);
          currentTraining.get('overRidePriceReason').setValue(this.birdValueReason);
        } else {
          this.pricingFormSubmitted = true
        }
      } else if (this.freeOffer) {
        currentTraining.get('overRidePrice').setValue(0);
        currentTraining.get('overRidePercent').setValue(100);
        currentTraining.get('isOverRidePrice').setValue(true);
        currentTraining.get('overRidePriceReason').setValue(this.birdValueReason);
      } else {
        let price = this.cart?.trainings[this.overRidePriceTrainingIndex].birdPrice && this.cart?.trainings[this.overRidePriceTrainingIndex].birdPrice != '0' && this.checkBirdPriceAvailablity(this.cart?.trainings[this.overRidePriceTrainingIndex].expiryDateBirdPrice) ? this.cart?.trainings[this.overRidePriceTrainingIndex].birdPrice : (this.cart?.trainings[this.overRidePriceTrainingIndex].price && this.cart?.trainings[this.overRidePriceTrainingIndex].price != '0' ? this.cart?.trainings[this.overRidePriceTrainingIndex].price : 0);
        currentTraining.get('overRidePrice').setValue(price);
        currentTraining.get('overRidePercent').setValue(0);
        currentTraining.get('isOverRidePrice').setValue(false);
        currentTraining.get('overRidePriceReason').setValue('');
      }
    ...
    if (this.isTrainingManualOverride && this.overRidePriceTrainingIndex != null) {
      ...
      if (this.alternatePricing) {
        if (this.alternatePricingForm.valid) {
          currentTraining.get('manualOverRidePrice').setValue(this.birdValue);
          currentTraining.get('manualOverRidePercent').setValue(this.birdPercentage);
          currentTraining.get('manualIsOverRidePrice').setValue(true);
          currentTraining.get('manualoverRidePriceReason').setValue(this.birdValueReason);
        } else {
          this.pricingFormSubmitted = true
        }
      } else if (this.freeOffer) {
        currentTraining.get('manualOverRidePrice').setValue(0);
        currentTraining.get('manualOverRidePercent').setValue(100);
        currentTraining.get('manualIsOverRidePrice').setValue(true);
        currentTraining.get('manualoverRidePriceReason').setValue(this.birdValueReason);
      } else {
        let price = (this.cart?.trainings[this.overRidePriceTrainingIndex].manual.birdPrice && this.cart?.trainings[this.overRidePriceTrainingIndex].manual.birdPrice != '0') ? this.cart?.trainings[this.overRidePriceTrainingIndex].manual.birdPrice : this.cart?.trainings[this.overRidePriceTrainingIndex].manual.price;
        currentTraining.get('manualOverRidePrice').setValue(price);
        currentTraining.get('manualOverRidePercent').setValue(0);
        currentTraining.get('manualIsOverRidePrice').setValue(false);
        currentTraining.get('manualoverRidePriceReason').setValue('');
      }
    }
    else if (this.overRidePriceManualIndex != null) {
      let currentManual = this.cartManuals.at(this.overRidePriceManualIndex);
      if (this.alternatePricing) {
        if (this.alternatePricingForm.valid) {
          currentManual.get('overRidePrice').setValue(this.birdValue);
          currentManual.get('overRidePercent').setValue(this.birdPercentage);
          currentManual.get('isOverRidePrice').setValue(true);
          currentManual.get('overRidePriceReason').setValue(this.birdValueReason);
        } else {
          this.pricingFormSubmitted = true
        }
      } else if (this.freeOffer) {
        currentManual.get('overRidePrice').setValue(0);
        currentManual.get('overRidePercent').setValue(100);
        currentManual.get('isOverRidePrice').setValue(true);
        currentManual.get('overRidePriceReason').setValue(this.birdValueReason);
      } else {
        let price = (this.cart?.manuals[this.overRidePriceManualIndex]?.discountPrice && this.cart?.manuals[this.overRidePriceManualIndex]?.discountPrice != 0) ? this.cart?.manuals[this.overRidePriceManualIndex]?.discountPrice : this.cart?.manuals[this.overRidePriceManualIndex]?.price
        currentManual.get('overRidePrice').setValue(price);
        currentManual.get('overRidePercent').setValue(0);
        currentManual.get('isOverRidePrice').setValue(false);
        currentManual.get('overRidePriceReason').setValue('');
      }
    }
    if (!this.pricingFormSubmitted) {
      this.updateCartForm();
  ...
  getCartUserData(data) {
    if (data) {
      this.threadApi.updateCartItems({ cartId: this.cart.cartId, userId: data[0].userId }).subscribe((resp: any) => {
        this.commonService.cartUpdateSubject.next(resp?.data);
      }, (error: any) => {
        console.error("error: ", error);
      });
