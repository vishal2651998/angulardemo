...
@Component({
  selector: 'app-manuals-payment-controller',
  templateUrl: './manuals-payment-controller.component.html',
  styleUrls: ['./manuals-payment-controller.component.scss']
})
export class ManualsPaymentControllerComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.shippingTo = this.firstNameValue + " " + this.lastNameValue;
    ...
    this.addressLine1Value = this.formValue?.shippingAddress1;
    this.addressLine2Value = this.formValue?.shippingAddress2;
    this.cityValue = this.formValue?.shippingCity;
    this.zipValue = this.formValue?.shippingZip;
    this.shippingAdress = this.addressLine1Value  +  (this.addressLine2Value?' '+this.addressLine2Value:'' ) + ', '  + this.cityValue + ", " +this.zipValue;
  ...
  calculateTotalAmount() {
    return this.shippingCost + this.salesTax + this.shippingTax + parseInt(this.buyManualCount) * (this.discountPrice ? parseFloat(this.discountPrice) : parseFloat(this.payablePrice));
  }
  ...
  paymentByCheckSubmit() {
    ...
    let stateDataValue = this.stateDropdownData?.filter((state: any) => {
      return state.id == this.stateValue;
    });
    if (stateDataValue) stateName = stateDataValue[0]?.name;
    let countryDataValue = this.countryDropdownData?.filter((country: any) => {
      return country.id == this.countryValue;
    });
    if (countryDataValue) countryName = countryDataValue[0]?.name;
    let response: any = {
  ...
  loadCountryStateData() {
    this.threadApi.countryMasterData().subscribe((response: any) => {
      if (response.status == "Success") {
        ...
        let stateDataValue = this.stateDropdownData?.filter((state: any) => {
          return state.name == this.formValue?.shippingState;
        });
        if (stateDataValue) {
