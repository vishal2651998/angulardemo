...
@Component({
  selector: "app-knowledge-articles-solr",
  templateUrl: "./knowledge-articles-solr.component.html",
  styleUrls: ["./knowledge-articles-solr.component.scss"],
  styles: [
    `
      .masonry-item {
        width: 268px;
...
export class KnowledgeArticlesSolrComponent implements OnInit, OnDestroy {
    ...
    if(url[1] == RedirectionPage.Search ) {
      ...
      if(localStorage.getItem('KACategoryEnable') != "1"){
       ...
       // localStorage.removeItem('currentWorkstream');        
        ...
        setTimeout(() => { 
          this.catId = "1";
          this.catName ="All Knowledge Articles";
  ...
  ngOnInit(): void {
    ...
    this.subscription.add(
      this.sharedSvc._OnLayoutStatusReceivedSubject.subscribe((r, r1 = "") => {
        ...
        if(r['access'] != 'thread') {
          if(catpageRefresh && (action == 'updateLayout'  || action == 'updateLayout-knowledgearticles')){
  ...
  loadKAPageSolr(searchValue,listingSolr=0)
  {
    ...
    if(this.pageDataInfo == pageInfo.knowledgeArticlePage){
      let catidPush = [];
      catidPush.push(this.catId);
      if(this.catId != ''){
        if(this.catId == "-1"){
          apiData["filters"]["folderIdsStrArr"]= catidPush;
        }
      ...
      console.log(this.knowledgeArticleFilterOptions)
      if(this.knowledgeArticleFilterOptions["katype"] == 'pined') {
        let arrayPin=[];
        arrayPin.push(this.userId);
        apiData["filters"]["pinedUsersStrArr"]=arrayPin;
      }
