...
this.getrecentViews('');
  ...
  apiFormData.append('type', "1");  // recent view
    ...
    for (let rcv in recentViewsArr)
        {
         ...
         let risdomainId=recentValuearr.domainId;
         ...
         let rthreadCatg = (recentValuearr.threadCategory) ? 0 : recentValuearr.threadCategory;
...
recentViewClick(event,typeId: any,Id,typeDomainId)
  {
    console.log(typeId)
    let flag: any = true;
    ...
    switch(typeId) {
      case 2:
        ...
        if(this.domainId=='317')
        {
          url = `${view}${Id}/${typeDomainId}`;
        }
        else
        {
          url = `${view}${Id}`;
