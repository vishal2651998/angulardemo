...
@Component({
  selector: 'app-recent-tech-info-widgets',
  templateUrl: './recent-tech-info-widgets.component.html',
  styleUrls: ['./recent-tech-info-widgets.component.scss'],
  animations: [
    trigger('anim', [
       transition('* => void', [
...
export class RecentTechInfoWidgetsComponent implements OnInit {
...
public loadingTechInfo:boolean=false;
  ...
  ngOnInit(): void {
    ...
    var landingpage_attr3=localStorage.getItem('landingpage_attr9');
...
this.getrecentViews('');
  ...
  apiFormData.append('type', "3");  // recent tech info
  ...
  this.LandingpagewidgetsAPI.GetRecentViews(apiFormData).subscribe((response) => {
   ...
   this.loadingTechInfo=false;
   if(recentviewtotal>0)
   {
this.norecentViews=false;
this.moreTechInfo=true;
   }
   else
   {
...
navigateTosupportPage(event,id) {
 ...
 localStorage.setItem('fromHometoDocuments','1');
