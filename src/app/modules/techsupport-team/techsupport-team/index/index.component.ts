...
export class IndexComponent implements OnInit {
  ...
  teamForm: FormGroup;
  levelForm: FormGroup;
  public title: string = "Tech Support Team";
   ...
   // this.bodyElem.classList.add(this.bodyClass);
    ...
    this.setScreenHeight();
      ...
      this.subscription.add(
        this.commonService.NewButtonHeaderCallReceivedSubject.subscribe((r) => {
          this.navigateWindow('add','0');
        })
      );
  ...
  techsupportMenus(type='',id=''){
    ...
    //}
    this.landingpageAPI.apiTechSupportMenusAPI(apiFormData).subscribe((response) => {
        ...
        this.ticketStatusList = response.ticketStatusList;
          if(id == '' || type == 'team'){
            ...
            for (let i in this.teamList) {
              this.teamList[i].activeClass = false;
              if(this.teamId == this.teamList[i].id){
                this.teamList[i].activeClass = true;
              }
            }
      ...
      setTimeout(() => {
        let secElement1 = document.getElementById('step');
        let secElement = document.getElementById('teamid-'+this.teamId);
        let scrollTop = secElement.offsetTop;
        console.log(secElement1)
        console.log(scrollTop)
        setTimeout(() => {
          secElement1.scrollTop = scrollTop;
        }, 200);
      }, 700);
    });
  ...
  applySearch(action,val) {
    if(action == 'emit'){
        ...
        this.pageRefresh['searchKey'] = this.searchVal;
  ...
  selectTap(index,id){
    ...
    this.teamList[index].activeClass = true;
    this.pageRefresh['teamId'] = id;
    this.teamId = id;
    this.pageRefresh['action'] = 'api';
    this.actionMoreFlag = false;
    console.log(this.threadOrderType,this.teamId,this.ticketStatus,this.teamMemberId);
...
/**
 ...
 setusDefultTeam(id,index){
    const apiFormData = new FormData();
    apiFormData.append("apiKey", Constant.ApiKey,);
    apiFormData.append("domainId", this.domainId);
    apiFormData.append("countryId", this.countryId);
    apiFormData.append("userId", this.userId);
    apiFormData.append("teamId", id);
    this.landingpageAPI.updateDefaultTeam(apiFormData).subscribe((response) => {
        console.log(response);
        if (response.status == "Success") {
          for (let i in this.teamList) {
            this.teamList[i].isDefault = "0";
          }
          this.teamList[index].isDefault = "1";
          localStorage.setItem('defaultTechSupportTeamId', id);
        }
    });
 }
  // New Window
  navigateWindow(type,id,index='') {
    let url = "";
    localStorage.setItem('navFromUrl', 'techsupport-team' );
    if(type == 'add'){
      url = "techsupport-team/manage";
      this.router.navigate([url]);
    }
    else if(type == 'default'){
      this.setusDefultTeam(id,index);
    }
    else{
      url = "techsupport-team/manage/edit/"+id;
      this.router.navigate([url]);
    }
