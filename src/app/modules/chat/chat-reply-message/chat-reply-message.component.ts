...
@Component({
  selector: 'app-chat-reply-message',
  templateUrl: './chat-reply-message.component.html',
  styleUrls: ['./chat-reply-message.component.scss']
})
export class ChatReplyMessageComponent implements OnInit {
  ...
  GetMentionedContent(chat)
    ...
    redirectToReply(replychat) {
      let chatid = replychat.id
      let chat = document.getElementById(chatid);
      if (chat !=null && chat!=undefined)
      {
        document.getElementById(chatid).scrollIntoView();
        chat.classList.add('reply-to-chat-highlighted');
        setTimeout(() =>  chat.classList.remove('reply-to-chat-highlighted'),1000);
        // setTimeout(() =>  chat.classList.add('reply-to-chat-highlighted'),1000);
        // setTimeout(() =>  chat.classList.remove('reply-to-chat-highlighted'),1500);
      }else{
        let model_group = document.getElementById("ReplyPopup");
        model_group.classList.add('modal-fade')
        //this.OpenReplyChatPopup.emit(chat);
       this.chatservice.ReplyMessage = replychat;
      }
