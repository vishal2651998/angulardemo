...
export class PushNotificationsComponent implements OnInit {
  ...
  onRadioChange(id, pid, flag){
    ...
    for(let item in this.configData){
      if(this.configData[item].parentId == pid){
        for(let inneritem in this.configData[item].info){           
          let actionItems:any = {}; 
          actionItems['pid'] = pid;
          actionItems['id'] = this.configData[item].info[inneritem].id;
          if(this.configData[item].info[inneritem].id == id){
          ...
          this.configData[item].info[inneritem].toggleBox = actionItems.actionVal == '0' ? false : true;           
  ...
  onToggleBoxChange(action, id, pid, flag) {
    ...
    let type = '1';
    let actionItems:any = {id, pid, actionVal, type}; 
    this.getDistpatchNotificationConfig('update', actionItems);    
  }
